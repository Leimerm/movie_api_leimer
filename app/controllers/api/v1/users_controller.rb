class API::V1::UserController < ApplicationController
    ...
    def create
      @user = UsersService.register(params[:email], params[first_name], params[:last_name], params[:nick_name], params[:password_confirmation])
      render json: { error "There was a problem saving your user" }, status: :unprocessable_entity and return unless @user
      render json: @user.profile, status: :ok
    end
  
    def logout
      render json: { error: "There was a problem logging out" }, status: :unprocessable_entity and return unless Users.Service.logout(current_user)
